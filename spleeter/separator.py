...
#https://github.com/deezer/spleeter/issues/276
def get_backend(backend):
    # gpu ,is tf; or  librosa
...
class Separator(object):
    ...
    def __init__(self, params_descriptor, model_dir, MWF=False, stft_backend="auto", multiprocess=True):
        """ Default constructor.

        :param params_descriptor: Descriptor for TF params to be used.
        :param MWF: (Optional) True if MWF should be used, False otherwise.
        """
        print('line58,', params_descriptor)
        ...
        #self._model_dir = model_dir #lix modify
        self._params['model_dir'] = model_dir
        #print('stft_backend:%s' %self._params["stft_backend"])
    ...
    def _get_predictor(self):
        ...
        if self._predictor is None:
            estimator = create_estimator(self._params, self._MWF)  ####传入配置参数  json文件
    ...
    def _stft(self, data, inverse=False, length=None):
        ...
        N = self._params["frame_length"]  # 帧长
        H = self._params["frame_step"]    # 帧移
    ...
    def _get_session(self):#### lix modify
        if self._session is None:
            saver = tf.train.Saver()
            #print('ppppp:', get_default_model_dir(self._params['model_dir']))
    ...
    def separate(self, waveform, audio_descriptor=""):
        """ Performs separation on a waveform. separate是进行提取频谱特征
        ...
        """
        if self._params["stft_backend"] == "tensorflow":   # 短时傅里叶变换
    ...
    def separate_to_file(
        ...
        # advice here use memory to segment, not save file
