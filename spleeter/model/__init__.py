...
class InputProviderFactory(object): # 顶级分发
    ...
    def get(params):
        ...
        if stft_backend == "tensorflow":
            return WaveformInputProvider(params) #  use tf.signal.stft to get 频谱
        else:
            return SpectralInputProvider(params) #  librosa.stft to get 频谱
...
class EstimatorSpecBuilder(object):
    ...
    * In predict mode:      it takes as input and outputs waveform. The whole
    ...
    # Supported loss functions. 可选两种loss
    L1_MASK = 'L1_mask'
    WEIGHTED_L1_MASK = 'weighted_L1_mask'

    # Supported optimizers.   可选两种优化器
    ADADELTA = 'Adadelta'    # 自适应学习率
    SGD = 'SGD'
    ...
    def __init__(self, features, params):
        ...
        * In predict mode:      features is a dictionary with a "waveform"
        ...
        self._n_channels = params['n_channels'] #  通道数
        self._T = params['T'] #
        self._F = params['F'] #
        self._frame_length = params['frame_length']
        self._frame_step = params['frame_step']
        #print('=======tfls:', self._mix_name, ',', self._n_channels, ',', self._T, ',', self._F, ',', self._frame_length, ',', self._frame_step )
    ...
    def _build_model_outputs(self):
        """ Created a batch_size x T x F x n_channels input tensor containing
        ...
        input_tensor = self.spectrogram_feature  # 幅度谱作为输入
        ...
        self._model_outputs = apply_model( #  模型构建
            input_tensor,
            self._instruments,
    ...
    def _build_loss(self, labels):
        ...
        if loss_type == self.L1_MASK:
            losses = {
                name: tf.reduce_mean(tf.abs(output - labels[name])) # 平均绝对误差
    ...
    def _build_stft_feature(self): #  计算频谱以及幅度谱
    ...
    def _build_masks(self): #  计算模型输出的频谱图
        """
        ...
        separation_exponent = self._params['separation_exponent'] # e^2
    ...
    def _build_outputs(self):#
        if self.include_stft_computations(): # 使用tf后端计算 stft
...
## 参数列表固定   类似还有input_fn   特征  标签   模式  网络参数
